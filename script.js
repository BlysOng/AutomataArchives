...
let workflowModalShotResizeHandler = null;
...
/* ----------------------------------------------------------------
   ...
   the image is scaled to a "cover" fit (see fitShotImage below), so
   it's always at least as large as the box in both directions —
   zero letterboxing on any side, whatever the screenshot's own
   aspect ratio. Whichever axis ends up bigger than the box is what
   you drag to pan. Click/tap-and-drag (mouse or touch, via pointer
   events) pans across the full image instead of the browser's
   native scrollbar. Wired up once on the shared modal; works the
   same on every page (n8n/zapier/make).
   ---------------------------------------------------------------- */
function initShotDrag(shotWrap){
  ...
  shotWrap.addEventListener('pointerdown', (e) => {
    const canPanX = shotWrap.scrollWidth > shotWrap.clientWidth;
    const canPanY = shotWrap.scrollHeight > shotWrap.clientHeight;
    if (!canPanX && !canPanY) return; // image already fits exactly, nothing to pan
    isDown = true;
    moved = false;
    startX = e.clientX;
    startY = e.clientY;
    startScrollX = shotWrap.scrollLeft;
    startScrollY = shotWrap.scrollTop;
  ...
  shotWrap.addEventListener('pointermove', (e) => {
    ...
    shotWrap.scrollLeft = startScrollX - dx;
    shotWrap.scrollTop = startScrollY - dy;
  });
  function endDrag(e){
    if (!isDown) return;
    isDown = false;
...
/* Sizes the screenshot to a "cover" fit — the SAME math as CSS
   object-fit: cover (scale = the LARGER of the two fit ratios) —
   so the image is guaranteed to be at least as big as the box on
   BOTH axes at once. That's what actually eliminates letterboxing:
   scaling by height alone (or width alone) leaves a gap whenever an
   image's aspect ratio happens to sit close to the box's own, which
   is exactly the bug fixed here. Whichever axis ends up bigger than
   the box afterwards is left pannable via initShotDrag(); the image
   starts centered on open, like a centered object-fit: cover crop,
   rather than pinned to a corner. */
function fitShotImage(shotWrap, img){
  const cw = shotWrap.clientWidth;
  const ch = shotWrap.clientHeight;
  const nw = img.naturalWidth;
  const nh = img.naturalHeight;
  if (!cw || !ch || !nw || !nh) return;

  const scale = Math.max(cw / nw, ch / nh);
  const renderW = Math.ceil(nw * scale);
  const renderH = Math.ceil(nh * scale);
  img.style.width = renderW + 'px';
  img.style.height = renderH + 'px';

  // Start centered rather than pinned top-left.
  shotWrap.scrollLeft = Math.max(0, (renderW - cw) / 2);
  shotWrap.scrollTop = Math.max(0, (renderH - ch) / 2);

  shotWrap.classList.toggle('is-draggable',
    renderW > cw + 1 || renderH > ch + 1);
}
...
function openWorkflowModal(wf){
  ...
  // is scaled to a "cover" fit — see fitShotImage() below — so it
  // always fully fills the box on both axes with zero letterboxing,
  // whatever the screenshot's own aspect ratio. Whichever axis ends
  // up bigger than the box is pannable; the drag hint (only shown
  // once that's confirmed) tells the user they can drag to see the
  // rest. No wf.image yet -> show the placeholder instead.
  ...
  shotImg.style.width = '';
  shotImg.style.height = '';
  if (wf.image){
    shotImg.alt = sanitizeText(wf.title) + ' — workflow screenshot';
    shotImg.style.display = 'block';
    shotPlaceholder.style.display = 'none';
    const doFit = () => fitShotImage(shotWrap, shotImg);
    if (shotImg.complete && shotImg.src === wf.image){
      doFit();
    } else {
      shotImg.onload = doFit;
    }
    shotImg.src = wf.image;
    // Keeps the cover-fit correct if the viewport is resized (e.g.
    // rotating a tablet) while this modal is still open.
    window.removeEventListener('resize', workflowModalShotResizeHandler);
    workflowModalShotResizeHandler = () => { if (shotImg.style.display !== 'none') doFit(); };
    window.addEventListener('resize', workflowModalShotResizeHandler);
  } else {
    shotImg.onload = null;
...
function closeWorkflowModal(){
  ...
  if (workflowModalShotResizeHandler) window.removeEventListener('resize', workflowModalShotResizeHandler);
